...
void Minho_Robot::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
    ...
    if(!is_ros_initialized_){
        ...
        ros::SubscribeOptions control_info = ros::SubscribeOptions::create<mtmsl_common::controlInfo>(
        ...
        ros::SubscribeOptions teleop_info = ros::SubscribeOptions::create<mtmsl_common::teleop>(
        ...
        robot_info_pub_ = _node_ros_->advertise<mtmsl_common::robotInfo>(robotinfo_topic.str(),100);
...
std::vector<mtmsl_common::obstacle> Minho_Robot::detectObstacles()
{
    std::vector<mtmsl_common::obstacle>obstacles;
    obstacles.clear();
    
    physics::WorldPtr world = _model_->GetWorld(); 
    std::vector<physics::ModelPtr> models_list = world->GetModels();
    mtmsl_common::obstacle obs;
    double min = 0.0, max = 0.5;
    double ratio = 0.0;
    int type;
    bool isObstacle;
